		...
		if Q_runFlag_Caminar:
			if Q_Agent.S0 == 0 and Q_Agent.S1 == 3:
				...
				if Action == 'out of bounds':
					...
					#	continue
					# if Q_Agent.S0 == 2 and Q_Agent.S1 == 3:
					# 	raw_reward = 50#int(ser.readline())
					# else:
					# 	if Q_Agent.S0 == 1 and Q_Agent.S1 == 3:
					# 		raw_reward = -100
					# 	else:
...
while (not crawlerDone):
     ...
     #   print(ser.read(100))
		...
		elif RoboCmd == 'Caminar':
			epsilon=0
			if Q_runFlag_Caminar == False:
				Q_runFlag_Caminar = True	
				Q_go_Caminar = True
				Qthread = threading.Thread(target = Caminar)
				Qthread.start()
